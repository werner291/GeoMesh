        ...
        : linkMgr(linkMgr)
{
    ...
    fdnotifier.addFileDescriptor(socketID,
                                 std::bind(&UDPManager::handleIncoming, this,
                                           std::placeholders::_1));
...
void UDPManager::connectTo(std::string address, int port)
{
    ...
    helloMsg << "GeoMesh_UDP_Bridge_Hello cientIfaceID:" <<
...
void UDPManager::sendControlMessage(const std::string &msg,
                                    struct sockaddr_in &destAddr)
{
...
void UDPManager::handleIncoming(int fd)
{
    ...
    if (nbytes > 0)
    { // It's a proper datagaram (not an error)
        ...
        if (protVersion > 0)
        {
            Logger::log(LogLevel::WARN,
                        "Unsupported protocol version on UDP"
    ...
    } else
    {
        ...
        Logger::log(LogLevel::ERROR,
                    "UDPManager: receive error: " +
                    std::string(strerror(err)));
    }
}

void UDPManager::processNormalPacketFragment(const uint8_t *buffer, int nbytes,
                                             uint16_t localIface)
{
    ...
    {
        Logger::log(WARN,
                    "Received message for interface " +
                    std::to_string(localIface)
                                             ...
                                             sockaddr_in &sender)
{
                ...
                strlen("GeoMesh_UDP_Bridge_Hello")) == 0)
    {
        ...
        establishedLinks
                .insert(std::make_pair(newIface->getInterfaceId(), newIface));
