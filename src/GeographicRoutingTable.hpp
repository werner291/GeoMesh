 ...
 */
class GeographicRoutingTable {

    /**
     * An entry in the greedy routing table.
     *
     * Such an entry means "Sending a packet through the interface with
     * the specified iFaceID will bring the packet closer to the target
     * location or to a node that knows a way to get the packet closer."
     */
    struct Entry {
        GPSLocation target; // Where this routing table entry will lead.
    ...
    std::vector<Entry> mGreedyRoutingTable;

    std::vector<Entry>::iterator getClosestEntry(const GPSLocation&closestTo) {
        ...
        auto closestRule = GeographicRoutingTable::mGreedyRoutingTable.end();
...
public:
     ...
     */
    int getGreedyInterface(int fromInterface, const GPSLocation &destination, double maxDistance);

    bool insertIfUseful(GPSLocation suggestionTarget, int iFaceID, int hops, const GPSLocation& referenceLoc);
     ...
     *         location.
     */
    bool hasCloserEntry(const GPSLocation& target, double maxDistance) {
        for (auto itr = GeographicRoutingTable::mGreedyRoutingTable.begin(); itr != GeographicRoutingTable::mGreedyRoutingTable.end(); itr++) {
